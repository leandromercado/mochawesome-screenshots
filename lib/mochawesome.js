...
function Mochawesome (runner, options) {
  ...
  runner.on('test', function (test) {
    test.scrFileName = config.shortScrFileNames ? Date.now() + '.png' : (test.fullTitle().replace(/[^\w_\-]/g, '_').replace(/_+/g, '_').toLowerCase()).substr(0,config.screenshotMaxFileLenght) + '.png';
  });
